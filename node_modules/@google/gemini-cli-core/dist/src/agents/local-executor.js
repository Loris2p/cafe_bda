...
export class LocalAgentExecutor {
    ...
    async run(inputs, signal) {
        ...
        try {
            ...
            const startIndex = this.runtimeContext.userHintService.getLatestHintIndex();
            this.runtimeContext.userHintService.onUserHint(hintListener);
            try {
                const initialHints = this.runtimeContext.userHintService.getUserHintsAfter(startIndex);
                const formattedInitialHints = formatUserHintsForModel(initialHints);
                let currentMessage = formattedInitialHints
                    ? {
                        role: 'user',
    ...
    async createChatObject(inputs, tools) {
        ...
        const systemInstruction = promptConfig.systemPrompt
            ...
            : undefined;
        try {
            return new GeminiChat(this.runtimeContext, systemInstruction, [{ functionDeclarations: tools }], startHistory, undefined, undefined, 'subagent');
        }
        catch (e) {
            await reportError(e, `Error initializing Gemini chat for agent ${this.definition.name}.`, startHistory, 'startChat');
