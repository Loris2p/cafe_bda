...
describe('GeminiChat', () => {
    ...
    describe('sendMessageStream', () => {
        ...
        it('should flush transcript before tool dispatch for pure tool call with no text or thoughts', async () => {
            ...
            vi.mocked(mockContentGenerator.generateContentStream).mockResolvedValue(pureToolCallStream);
            const { default: fs } = await import('node:fs');
            const writeFileSync = vi.mocked(fs.writeFileSync);
            const writeCountBefore = writeFileSync.mock.calls.length;
            const stream = await chat.sendMessageStream({ model: 'test-model' }, 'analyze test.py', 'prompt-id-pure-tool-flush', new AbortController().signal, LlmRole.MAIN);
            for await (const _ of stream) {
                // consume
            }
            const newWrites = writeFileSync.mock.calls.slice(writeCountBefore);
            expect(newWrites.length).toBeGreaterThan(0);
            const lastWriteData = JSON.parse(newWrites[newWrites.length - 1][1]);
            const geminiMessages = lastWriteData.messages.filter((m) => m.type === 'gemini');
