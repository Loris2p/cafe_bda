...
import { Config, debugLogger, ExtensionDisableEvent, ExtensionEnableEvent, ExtensionInstallEvent, ExtensionLoader, ExtensionUninstallEvent, ExtensionUpdateEvent, getErrorMessage, getRealPath, logExtensionDisable, logExtensionEnable, logExtensionInstallEvent, logExtensionUninstall, logExtensionUpdateEvent, loadSkillsFromDir, loadAgentsFromDirectory, homedir, coreEvents, applyAdminAllowlist, getAdminBlockedMcpServersMessage, CoreToolCallStatus, loadExtensionPolicies, isSubpath, HookType, } from '@google/gemini-cli-core';
...
Would you like to attempt to install via "git clone" instead?`))) {
    ...
    async loadExtensions() {
        ...
        this.loadingPromise = (async () => {
            try {
                ...
                const builtExtensions = builtExtensionsOrNull.filter((ext) => ext !== null);
                const seenNames = new Set();
                for (const ext of builtExtensions) {
                    if (seenNames.has(ext.name)) {
                        throw new Error(`Extension with name ${ext.name} already was loaded.`);
                    }
                    seenNames.add(ext.name);
                }
                this.loadedExtensions = builtExtensions;
                await Promise.all(this.loadedExtensions.map((ext) => this.maybeStartExtension(ext)));
                return this.loadedExtensions;
            }
            finally {
    ...
    async loadExtension(extensionDir) {
        ...
        if (this.getExtensions().find((installed) => installed.name === extension.name)) {
            throw new Error(`Extension with name ${extension.name} already was loaded.`);
        }
    ...
    async _buildExtension(extensionDir) {
        ...
        try {
            ...
            const contextFiles = getContextFileNames(config)
                .map((contextFileName) => {
                const contextFilePath = path.join(effectiveExtensionPath, contextFileName);
                if (!isSubpath(effectiveExtensionPath, contextFilePath)) {
                    throw new Error(`Invalid context file path: "${contextFileName}". Context files must be within the extension directory.`);
                }
            ...
            if (fs.existsSync(policyDir)) {
                ...
                if (result.errors.length > 0) {
                    for (const error of result.errors) {
                        debugLogger.warn(`[ExtensionManager] Error loading policies from ${config.name}: ${error.message}${error.details ? `\nDetails: ${error.details}` : ''}`);
                    }
                }
            }
