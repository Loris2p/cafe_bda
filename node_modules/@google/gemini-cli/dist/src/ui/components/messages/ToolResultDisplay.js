...
export const ToolResultDisplay = ({ resultDisplay, availableTerminalHeight, terminalWidth, renderOutputAsMarkdown = true, maxLines, hasFocus = false, }) => {
    ...
    const renderVirtualizedAnsiLine = React.useCallback(({ item }) => (_jsx(Box, { height: 1, overflow: "hidden", children: _jsx(AnsiLineText, { line: item }) })), []);
    ...
    if (typeof truncatedResultDisplay === 'object' &&
    ...
    if (isAlternateBuffer && Array.isArray(truncatedResultDisplay)) {
        ...
        truncatedResultDisplay.length, limit);
        return (_jsx(Box, { width: childWidth, flexDirection: "column", maxHeight: listHeight, children: _jsx(ScrollableList, { width: childWidth, 
                // eslint-disable-next-line @typescript-eslint/no-unsafe-type-assertion
                data: truncatedResultDisplay, renderItem: renderVirtualizedAnsiLine, estimatedItemHeight: () => 1, keyExtractor: keyExtractor, initialScrollIndex: SCROLL_TO_ITEM_END, hasFocus: hasFocus }) }));
    }
    ...
    if (formattedJSON) {
        ...
        , { 
            ...
            // eslint-disable-next-line @typescript-eslint/no-unsafe-type-assertion
            filename: truncatedResultDisplay.fileName, availableTerminalHeight: availableHeight, terminalWidth: childWidth }));
    }
    else {
        const shouldDisableTruncation = isAlternateBuffer ||
            (availableTerminalHeight === undefined && maxLines === undefined);
        content = (_jsx(AnsiOutputText
        // eslint-disable-next-line @typescript-eslint/no-unsafe-type-assertion
        , { 
            // eslint-disable-next-line @typescript-eslint/no-unsafe-type-assertion
            data: truncatedResultDisplay, availableTerminalHeight: isAlternateBuffer ? undefined : availableHeight, width: childWidth, maxLines: isAlternateBuffer ? undefined : maxLines, disableTruncation: shouldDisableTruncation }));
    }
    // 4. Final render based on session mode
    if (isAlternateBuffer) {
        return (_jsx(Scrollable, { width: childWidth, maxHeight: maxLines ?? availableHeight, hasFocus: hasFocus, scrollToBottom: true, children: content }));
    }
    return (_jsx(Box, { width: childWidth, flexDirection: "column", children: _jsx(MaxSizedBox, { maxHeight: availableHeight, maxWidth: childWidth, additionalHiddenLinesCount: hiddenLinesCount, children: content }) }));
};
