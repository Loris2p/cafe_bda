...
describe('Gradient Crash Regression Tests', () => {
    it('<Header /> should not crash when theme.ui.gradient is empty', async () => {
        const { lastFrame, waitUntilReady, unmount } = renderWithProviders(_jsx(Header, { version: "1.0.0", nightly: false }), {
            width: 120,
        });
    ...
    it('<ModelDialog /> should not crash when theme.ui.gradient is empty', async () => {
        const { lastFrame, waitUntilReady, unmount } = renderWithProviders(_jsx(ModelDialog, { onClose: async () => { } }), {
            width: 120,
        });
        await waitUntilReady();
        expect(lastFrame()).toBeDefined();
    ...
    it('<Banner /> should not crash when theme.ui.gradient is empty', async () => {
        const { lastFrame, waitUntilReady, unmount } = renderWithProviders(_jsx(Banner, { bannerText: "Test Banner", isWarning: false, width: 80 }), {
            width: 120,
        });
        await waitUntilReady();
        expect(lastFrame()).toBeDefined();
    ...
    it('<StatsDisplay /> should not crash when theme.ui.gradient is empty', async () => {
        const { lastFrame, waitUntilReady, unmount } = renderWithProviders(_jsx(StatsDisplay, { duration: "1s", title: "My Stats" }), {
            width: 120,
            uiState: {
                sessionStats: mockSessionStats,
            },
