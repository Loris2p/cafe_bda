...
describe('MultiFolderTrustDialog', () => {
    ...
    it('renders the dialog with the list of folders', async () => {
        ...
        const { lastFrame, waitUntilReady, unmount } = render(_jsx(MultiFolderTrustDialog, { ...defaultProps, folders: folders }));
    ...
    it('calls onComplete and finishAddingDirectories with an error on escape', async () => {
        ...
        const { waitUntilReady, unmount } = render(_jsx(MultiFolderTrustDialog, { ...defaultProps, folders: folders }));
        await waitUntilReady();
    ...
    it('adds directories to workspace context when "Yes" is chosen', async () => {
        const folders = ['/path/to/folder1', '/path/to/folder2'];
        const { waitUntilReady, unmount } = render(_jsx(MultiFolderTrustDialog, { ...defaultProps, folders: folders, trustedDirs: ['/already/trusted'] }));
        await waitUntilReady();
        const { onSelect } = mockedRadioButtonSelect.mock.calls[0][0];
        await act(async () => {
    ...
    it('shows an error message and completes when config is missing', async () => {
        const folders = ['/path/to/folder1'];
        const { waitUntilReady, unmount } = render(_jsx(MultiFolderTrustDialog, { ...defaultProps, folders: folders, config: null }));
        await waitUntilReady();
        const { onSelect } = mockedRadioButtonSelect.mock.calls[0][0];
        await act(async () => {
            onSelect(MultiFolderTrustChoice.YES);
    ...
    it('collects and reports errors when some directories fail to be added', async () => {
        ...
        const { waitUntilReady, unmount } = render(_jsx(MultiFolderTrustDialog, { ...defaultProps, folders: folders, errors: ['initial error'] }));
        await waitUntilReady();
        const { onSelect } = mockedRadioButtonSelect.mock.calls[0][0];
        await act(async () => {
            onSelect(MultiFolderTrustChoice.YES);
