...
vi.mock('@google/gemini-cli-core', async (importOriginal) => {
    ...
    return mockCoreDebugLogger(await importOriginal(), {
        stripAnsi: false,
    });
});
    ...
    describe('handleList', () => {
        it('should log a message if no extensions are installed', async () => {
            ...
            mockExtensionManager.prototype.loadExtensions = vi
                ...
                .mockResolvedValue([]);
            await handleList();
            expect(coreEvents.emitConsoleLog).toHaveBeenCalledWith('log', 'No extensions installed.');
            mockCwd.mockRestore();
        });
        ...
        it('should list all installed extensions', async () => {
            ...
            await handleList();
            expect(coreEvents.emitConsoleLog).toHaveBeenCalledWith('log', 'ext1@1.0.0\n\next2@2.0.0');
            mockCwd.mockRestore();
        });
        it('should list all installed extensions in JSON format', async () => {
            ...
            mockExtensionManager.prototype.loadExtensions = vi
                ...
                .mockResolvedValue(extensions);
            await handleList({ outputFormat: 'json' });
            expect(coreEvents.emitConsoleLog).toHaveBeenCalledWith('log', JSON.stringify(extensions, null, 2));
            mockCwd.mockRestore();
        });
        it('should log an error message and exit with code 1 when listing fails', async () => {
            ...
            await handleList();
            expect(coreEvents.emitConsoleLog).toHaveBeenCalledWith('error', 'List failed message');
