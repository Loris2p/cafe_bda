...
class GoogleSheetsService {
  // Version 6 : On utilise le constructeur standard
  late final GoogleSignIn _googleSignIn;

  GoogleSheetsService() {
    final serverClientId = dotenv.env['GOOGLE_SERVER_CLIENT_ID'] ?? dotenv.env['GOOGLE_CLIENT_ID'];
    final webClientId = dotenv.env['GOOGLE_WEB_CLIENT_ID'] ?? serverClientId;
    
    // On ne touche pas à Platform si on est sur le Web
    final isAndroid = !kIsWeb && Platform.isAndroid;
    
    _googleSignIn = GoogleSignIn(
      serverClientId: (kIsWeb || isAndroid) ? serverClientId : null,
      clientId: (!isAndroid) ? webClientId : null,
  ...
  /// Met à jour le client Sheets API avec un nouveau token d'accès.
  ...
  Future<bool> tryAutoAuthenticate() async {
    ...
    if (kIsWeb || Platform.isAndroid || Platform.isIOS) {
      try {
        // Version 6 : signInSilently() standard
  ...
  Future<String?> _authenticateMobile() async {
    try {
      // Version 6 : signIn() standard
      final account = await _googleSignIn.signIn();
      
      if (account == null) return 'Authentification annulée';
