...
describe('extension tests', () => {
    ...
    describe('loadExtensions', () => {
        ...
        it('should throw an error if a context file path is outside the extension directory', async () => {
            ...
            expect(consoleSpy).toHaveBeenCalledWith(expect.stringContaining('traversal-extension: Invalid context file path: "../secret.txt"'));
...
priority = 100
`;
            fs.writeFileSync(path.join(policiesDir, 'policies.toml'), policiesContent);
            const extensions = await extensionManager.loadExtensions();
            expect(extensions).toHaveLength(1);
            const extension = extensions[0];
            expect(extension.rules).toBeDefined();
            expect(extension.rules).toHaveLength(2);
            expect(extension.rules.find((r) => r.toolName === 'deny_tool')?.decision).toBe('deny');
            expect(extension.rules.find((r) => r.toolName === 'ask_tool')?.decision).toBe('ask_user');
            // Verify source is prefixed
            expect(extension.rules[0].source).toContain('Extension (policy-extension):');
        });
...
`;
            ...
            expect(consoleSpy).toHaveBeenCalledWith(expect.stringContaining('attempted to contribute an ALLOW rule'));
            expect(consoleSpy).toHaveBeenCalledWith(expect.stringContaining('attempted to contribute a rule for YOLO mode'));
            expect(consoleSpy).toHaveBeenCalledWith(expect.stringContaining('attempted to contribute a safety checker for YOLO mode'));
            consoleSpy.mockRestore();
        });
