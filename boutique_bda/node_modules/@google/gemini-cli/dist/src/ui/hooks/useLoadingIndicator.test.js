...
describe('useLoadingIndicator', () => {
    ...
    const renderLoadingIndicatorHook = (initialStreamingState, initialShouldShowFocusHint = false, initialRetryStatus = null, loadingPhrasesMode = 'all', initialErrorVerbosity = 'full') => {
        let hookResult;
        function TestComponent({ streamingState, shouldShowFocusHint, retryStatus, mode, errorVerbosity, }) {
        ...
        const { rerender } = render(_jsx(TestComponent, { streamingState: initialStreamingState, shouldShowFocusHint: initialShouldShowFocusHint, retryStatus: initialRetryStatus, mode: loadingPhrasesMode, errorVerbosity: initialErrorVerbosity }));
        return {
            result: {
                get current() {
                    return hookResult;
            ...
            rerender: (newProps) => rerender(_jsx(TestComponent, { mode: loadingPhrasesMode, errorVerbosity: initialErrorVerbosity, ...newProps })),
        };
    };
    ...
    it('should hide low-verbosity retry status for early retry attempts', () => {
        ...
        const { result } = renderLoadingIndicatorHook(StreamingState.Responding, false, retryStatus, 'all', 'low');
        expect(result.current.currentLoadingPhrase).not.toBe("This is taking a bit longer, we're still on it.");
    });
    it('should show a generic retry phrase in low error verbosity mode for later retries', () => {
        ...
        const { result } = renderLoadingIndicatorHook(StreamingState.Responding, false, retryStatus, 'all', 'low');
        expect(result.current.currentLoadingPhrase).toBe("This is taking a bit longer, we're still on it.");
    });
