...
vi.mock('./GeminiSpinner.js', () => ({
    GeminiSpinner: ({ altText }) => (_jsxs(Text, { children: ["GeminiSpinner ", altText] })),
}));
describe('GeminiRespondingSpinner', () => {
    ...
    it('renders spinner when responding', async () => {
        ...
        const { lastFrame, waitUntilReady, unmount } = render(_jsx(GeminiRespondingSpinner, {}));
    ...
    it('renders non-responding display when provided', async () => {
        mockUseStreamingContext.mockReturnValue(StreamingState.Idle);
        const { lastFrame, waitUntilReady, unmount } = render(_jsx(GeminiRespondingSpinner, { nonRespondingDisplay: "Waiting..." }));
        await waitUntilReady();
