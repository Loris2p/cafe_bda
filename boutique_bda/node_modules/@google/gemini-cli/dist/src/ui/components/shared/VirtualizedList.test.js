...
describe('<VirtualizedList />', () => {
    ...
    describe('with 10px height and 100 items', () => {
        ...
        it.each([
            ...
            const { lastFrame, waitUntilReady, unmount } = render(_jsx(Box, { height: 10, width: 100, borderStyle: "round", children: _jsx(VirtualizedList, { data: longData, renderItem: renderItem1px, keyExtractor: keyExtractor, estimatedItemHeight: () => itemHeight, initialScrollIndex: initialScrollIndex }) }));
        ...
        it('sticks to bottom when new items added', async () => {
            const { lastFrame, rerender, waitUntilReady, unmount } = render(_jsx(Box, { height: 10, width: 100, borderStyle: "round", children: _jsx(VirtualizedList, { data: longData, renderItem: renderItem1px, keyExtractor: keyExtractor, estimatedItemHeight: () => itemHeight, initialScrollIndex: 99 }) }));
            await waitUntilReady();
        ...
        it('scrolls down to show new items when requested via ref', async () => {
            const ref = createRef();
            const { lastFrame, waitUntilReady, unmount } = render(_jsx(Box, { height: 10, width: 100, borderStyle: "round", children: _jsx(VirtualizedList, { ref: ref, data: longData, renderItem: renderItem1px, keyExtractor: keyExtractor, estimatedItemHeight: () => itemHeight }) }));
            await waitUntilReady();
        ...
        it.each([
            ...
            const { lastFrame, waitUntilReady, unmount } = render(_jsx(Box, { height: 20, width: 100, borderStyle: "round", children: _jsx(VirtualizedList, { data: veryLongData, renderItem: ({ item }) => (_jsx(ItemWithEffect, { item: item }, item)), keyExtractor: keyExtractor, estimatedItemHeight: () => tallItemHeight, initialScrollIndex: initialScrollIndex }) }));
            await waitUntilReady();
            const frame = lastFrame();
    ...
    it('renders more items when a visible item shrinks via context update', async () => {
        ...
        const { lastFrame, waitUntilReady, unmount } = render(_jsx(TestComponent, {}));
        await waitUntilReady();
    ...
    it('updates scroll position correctly when scrollBy is called multiple times in the same tick', async () => {
        ...
        const renderItem1px = ({ item }) => (_jsx(Box, { height: itemHeight, children: _jsx(Text, { children: item }) }));
        const keyExtractor = (item) => item;
        const { waitUntilReady, unmount } = render(_jsx(Box, { height: 10, width: 100, borderStyle: "round", children: _jsx(VirtualizedList, { ref: ref, data: longData, renderItem: renderItem1px, keyExtractor: keyExtractor, estimatedItemHeight: () => itemHeight }) }));
        await waitUntilReady();
    ...
    it('renders correctly in copyModeEnabled when scrolled', async () => {
        ...
        const longData = Array.from({ length: 100 }, (_, i) => `Item ${i}`);
        // Use copy mode
        const { lastFrame, waitUntilReady, unmount } = render(_jsx(Box, { height: 10, width: 100, children: _jsx(VirtualizedList, { data: longData, renderItem: ({ item }) => (_jsx(Box, { height: 1, children: _jsx(Text, { children: item }) })), keyExtractor: (item) => item, estimatedItemHeight: () => 1, initialScrollIndex: 50 }) }));
        await waitUntilReady();
