...
export function TabHeader({ tabs, currentIndex, completedIndices = new Set(), showArrows = true, showStatusIcons = true, renderStatusIcon, }) {
    ...
    return (_jsxs(Box, { flexDirection: "row", marginBottom: 1, "aria-role": "tablist", children: [showArrows && _jsx(Text, { color: theme.text.secondary, children: '← ' }), tabs.map((tab, i) => (_jsxs(React.Fragment, { children: [i > 0 && _jsx(Text, { color: theme.text.secondary, children: ' │ ' }), showStatusIcons && (_jsxs(Text, { color: theme.text.secondary, children: [getStatusIcon(tab, i), " "] })), _jsx(Box, { maxWidth: i !== currentIndex ? 16 : 100, children: _jsx(Text, { color: i === currentIndex ? theme.status.success : theme.text.secondary, bold: i === currentIndex, underline: i === currentIndex, "aria-current": i === currentIndex ? 'step' : undefined, wrap: "truncate", children: tab.header }) })] }, tab.key))), showArrows && _jsx(Text, { color: theme.text.secondary, children: ' →' })] }));
}
