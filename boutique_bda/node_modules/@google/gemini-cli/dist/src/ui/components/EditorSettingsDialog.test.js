...
describe('EditorSettingsDialog', () => {
    ...
    it('renders correctly', async () => {
        const { lastFrame, waitUntilReady } = renderWithProvider(_jsx(EditorSettingsDialog, { onSelect: vi.fn(), settings: mockSettings, onExit: vi.fn() }));
        await waitUntilReady();
        expect(lastFrame()).toMatchSnapshot();
    });
    it('calls onSelect when an editor is selected', async () => {
        const onSelect = vi.fn();
        const { lastFrame, waitUntilReady } = renderWithProvider(_jsx(EditorSettingsDialog, { onSelect: onSelect, settings: mockSettings, onExit: vi.fn() }));
        await waitUntilReady();
        expect(lastFrame()).toContain('VS Code');
    });
    it('switches focus between editor and scope sections on Tab', async () => {
        const { lastFrame, stdin, waitUntilReady } = renderWithProvider(_jsx(EditorSettingsDialog, { onSelect: vi.fn(), settings: mockSettings, onExit: vi.fn() }));
        await waitUntilReady();
    ...
    it('calls onExit when Escape is pressed', async () => {
        const onExit = vi.fn();
        const { stdin, waitUntilReady } = renderWithProvider(_jsx(EditorSettingsDialog, { onSelect: vi.fn(), settings: mockSettings, onExit: onExit }));
        await waitUntilReady();
        await act(async () => {
            stdin.write('\u001B'); // Escape
        });
    ...
    it('shows modified message when setting exists in other scope', async () => {
        ...
        const { lastFrame, waitUntilReady } = renderWithProvider(_jsx(EditorSettingsDialog, { onSelect: vi.fn(), settings: settingsWithOtherScope, onExit: vi.fn() }));
        await waitUntilReady();
        const frame = lastFrame() || '';
