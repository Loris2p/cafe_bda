...
describe('<ToolsList />', () => {
    it('renders correctly with descriptions', async () => {
        const { lastFrame, waitUntilReady } = renderWithProviders(_jsx(ToolsList, { tools: mockTools, showDescriptions: true, terminalWidth: 40 }));
        await waitUntilReady();
        expect(lastFrame()).toMatchSnapshot();
    });
    it('renders correctly without descriptions', async () => {
        const { lastFrame, waitUntilReady } = renderWithProviders(_jsx(ToolsList, { tools: mockTools, showDescriptions: false, terminalWidth: 40 }));
        await waitUntilReady();
        expect(lastFrame()).toMatchSnapshot();
    });
    it('renders correctly with no tools', async () => {
        const { lastFrame, waitUntilReady } = renderWithProviders(_jsx(ToolsList, { tools: [], showDescriptions: true, terminalWidth: 40 }));
        await waitUntilReady();
        expect(lastFrame()).toMatchSnapshot();
    });
});
