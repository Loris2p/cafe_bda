...
describe('<HalfLinePaddedBox />', () => {
    ...
    it('renders standard background and blocks when not iTerm2', async () => {
        ...
        const { lastFrame, waitUntilReady, unmount } = renderWithProviders(_jsx(HalfLinePaddedBox, { backgroundBaseColor: "blue", backgroundOpacity: 0.5, children: _jsx(Text, { children: "Content" }) }), { width: 10 });
    ...
    it('renders nothing when useBackgroundColor is false', async () => {
        const { lastFrame, waitUntilReady, unmount } = renderWithProviders(_jsx(HalfLinePaddedBox, { backgroundBaseColor: "blue", backgroundOpacity: 0.5, useBackgroundColor: false, children: _jsx(Text, { children: "Content" }) }), { width: 10 });
        await waitUntilReady();
        expect(lastFrame()).toMatchSnapshot();
        unmount();
    });
