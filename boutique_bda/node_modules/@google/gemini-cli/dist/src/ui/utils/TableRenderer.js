...
import { Text, Box, toStyledCharacters, styledCharsWidth, wordBreakStyledChars, wrapStyledChars, widestLineFromStyledChars, } from 'ink';
 ...
 */
const parseMarkdownToStyledChars = (text, defaultColor) => {
...
export const TableRenderer = ({ headers, rows, terminalWidth, }) => {
    const styledHeaders = useMemo(() => headers.map((header) => parseMarkdownToStyledChars(stripUnsafeCharacters(header), theme.text.link)), [headers]);
    const styledRows = useMemo(() => rows.map((row) => row.map((cell) => parseMarkdownToStyledChars(stripUnsafeCharacters(cell), theme.text.primary))), [rows]);
    ...
    const renderCell = (content, width, isHeader = false) => {
        ...
        return (_jsxs(Text, { children: [isHeader ? (_jsx(Text, { bold: true, color: theme.text.link, children: content.text })) : (_jsx(Text, { children: content.text })), ' '.repeat(paddingNeeded)] }));
    };
    ...
    const renderVisualRow = (cells, isHeader = false) => {
        ...
        return (_jsxs(Box, { flexDirection: "row", children: [_jsx(Text, { color: theme.border.default, children: "\u2502" }), renderedCells.map((cell, index) => (_jsxs(React.Fragment, { children: [_jsx(Box, { paddingX: 1, children: cell }), index < renderedCells.length - 1 && (_jsx(Text, { color: theme.border.default, children: "\u2502" }))] }, index))), _jsx(Text, { color: theme.border.default, children: "\u2502" })] }));
    };
