...
describe('skillUtils', () => {
    ...
    describe('linkSkill', () => {
        ...
        itif(process.platform !== 'win32')('should successfully link from a local directory', async () => {
        ...
        itif(process.platform !== 'win32')('should overwrite existing skill at destination', async () => {
            const mockSkillSourceDir = path.join(tempDir, 'mock-skill-source');
            const skillSubDir = path.join(mockSkillSourceDir, 'test-skill');
            await fs.mkdir(skillSubDir, { recursive: true });
            await fs.writeFile(path.join(skillSubDir, 'SKILL.md'), '---\nname: test-skill\ndescription: test\n---\nbody');
        ...
        it('should abort linking if consent is rejected', async () => {
            ...
            await expect(linkSkill(mockSkillSourceDir, 'workspace', () => { }, requestConsent)).rejects.toThrow('Skill linking cancelled by user.');
        ...
        it('should throw error if multiple skills with same name are discovered', async () => {
            ...
            await fs.writeFile(path.join(skillDir1, 'SKILL.md'), '---\nname: duplicate-skill\ndescription: desc1\n---\nbody1');
            await fs.writeFile(path.join(skillDir2, 'SKILL.md'), '---\nname: duplicate-skill\ndescription: desc2\n---\nbody2');
            await expect(linkSkill(mockSkillSourceDir, 'workspace', () => { })).rejects.toThrow('Duplicate skill name "duplicate-skill" found');
        });
    });
    it('should successfully install from a .skill file', async () => {
        ...
        const skills = await installSkill(skillPath, 'workspace', undefined, async () => { });
    ...
    it('should successfully install from a local directory', async () => {
        ...
        await fs.mkdir(skillSubDir, { recursive: true });
        await fs.writeFile(path.join(skillSubDir, 'SKILL.md'), '---\nname: test-skill\ndescription: test\n---\nbody');
        const skills = await installSkill(mockSkillDir, 'workspace', undefined, async () => { });
        expect(skills.length).toBe(1);
        expect(skills[0].name).toBe('test-skill');
    ...
    it('should abort installation if consent is rejected', async () => {
        ...
        await fs.writeFile(path.join(skillSubDir, 'SKILL.md'), '---\nname: test-skill\ndescription: test\n---\nbody');
        const requestConsent = vi.fn().mockResolvedValue(false);
        await expect(installSkill(mockSkillDir, 'workspace', undefined, async () => { }, requestConsent)).rejects.toThrow('Skill installation cancelled by user.');
        expect(requestConsent).toHaveBeenCalled();
