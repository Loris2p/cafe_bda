...
describe('ModelRouterService', () => {
    ...
    describe('route()', () => {
        ...
        it('should log a telemetry event on a successful decision', async () => {
            ...
            expect(ModelRoutingEvent).toHaveBeenCalledWith('strategy-chosen-model', 'test-router/fallback', 10, 'Strategy reasoning', false, undefined, ApprovalMode.DEFAULT, false, undefined);
        ...
        it('should log a telemetry event and return fallback on a failed decision', async () => {
            ...
            expect(ModelRoutingEvent).toHaveBeenCalledWith('default-model', 'router-exception', expect.any(Number), 'An exception occurred during routing.', true, 'Strategy failed', ApprovalMode.DEFAULT, false, undefined);
            expect(logModelRouting).toHaveBeenCalledWith(mockConfig, expect.any(ModelRoutingEvent));
        });
    });
});
