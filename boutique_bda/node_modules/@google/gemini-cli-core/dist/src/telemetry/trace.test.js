...
import { GeminiCliOperation, GEN_AI_CONVERSATION_ID, GEN_AI_AGENT_DESCRIPTION, GEN_AI_AGENT_NAME, GEN_AI_INPUT_MESSAGES, GEN_AI_OPERATION_NAME, GEN_AI_OUTPUT_MESSAGES, SERVICE_DESCRIPTION, SERVICE_NAME, } from './constants.js';
...
describe('runInDevTraceSpan', () => {
    ...
    it('should start an active span', async () => {
        ...
        const result = await runInDevTraceSpan({ operation: GeminiCliOperation.LLMCall }, fn);
        expect(result).toBe('result');
        expect(trace.getTracer).toHaveBeenCalled();
        expect(mockTracer.startActiveSpan).toHaveBeenCalledWith(GeminiCliOperation.LLMCall, {}, expect.any(Function));
    });
    it('should set default attributes on the span metadata', async () => {
        await runInDevTraceSpan({ operation: GeminiCliOperation.LLMCall }, async ({ metadata }) => {
            expect(metadata.attributes[GEN_AI_OPERATION_NAME]).toBe(GeminiCliOperation.LLMCall);
    ...
    it('should set span attributes from metadata on completion', async () => {
        ...
        expect(mockSpan.setAttribute).toHaveBeenCalledWith(GEN_AI_INPUT_MESSAGES, JSON.stringify({ query: 'hello' }));
        expect(mockSpan.setAttribute).toHaveBeenCalledWith(GEN_AI_OUTPUT_MESSAGES, JSON.stringify({ response: 'world' }));
    ...
    it('should handle errors in the wrapped function', async () => {
        ...
        await expect(runInDevTraceSpan({ operation: GeminiCliOperation.LLMCall }, async () => {
            throw error;
    ...
    it('should respect noAutoEnd option', async () => {
        let capturedEndSpan = () => { };
        const result = await runInDevTraceSpan({ operation: GeminiCliOperation.LLMCall, noAutoEnd: true }, async ({ endSpan }) => {
    ...
    it('should automatically end span on error even if noAutoEnd is true', async () => {
        ...
        await expect(runInDevTraceSpan({ operation: GeminiCliOperation.LLMCall, noAutoEnd: true }, async () => {
            throw error;
        })).rejects.toThrow(error);
        expect(mockSpan.end).toHaveBeenCalled();
    });
    it('should handle exceptions in endSpan gracefully', async () => {
        ...
        expect(mockSpan.setStatus).toHaveBeenCalledWith(expect.objectContaining({
            code: SpanStatusCode.ERROR,
