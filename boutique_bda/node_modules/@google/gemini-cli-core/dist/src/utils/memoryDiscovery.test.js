...
describe('memoryDiscovery', () => {
    ...
    describe('when untrusted', () => {
        it('does not load context files from untrusted workspaces', async () => {
            ...
            const result = flattenResult(await loadServerHierarchicalMemory(cwd, [], false, new FileDiscoveryService(projectRoot), new SimpleExtensionLoader([]), false));
        ...
        it('loads context from outside the untrusted workspace', async () => {
            await createTestFile(path.join(projectRoot, DEFAULT_CONTEXT_FILENAME), 'Project root memory');
            await createTestFile(path.join(cwd, DEFAULT_CONTEXT_FILENAME), 'Src directory memory');
            const filepathInput = path.join(homedir, GEMINI_DIR, DEFAULT_CONTEXT_FILENAME);
            const filepath = await createTestFile(filepathInput, 'default context content'); // In user home dir (outside untrusted space).
            const { fileCount, memoryContent, filePaths } = flattenResult(await loadServerHierarchicalMemory(cwd, [], false, new FileDiscoveryService(projectRoot), new SimpleExtensionLoader([]), false));
    ...
    it('should return empty memory and count if no context files are found', async () => {
        const result = flattenResult(await loadServerHierarchicalMemory(cwd, [], false, new FileDiscoveryService(projectRoot), new SimpleExtensionLoader([]), DEFAULT_FOLDER_TRUST));
        expect(result).toEqual({
            memoryContent: '',
            fileCount: 0,
            filePaths: [],
...
--- End of Context from: ${normMarker(path.relative(cwd, subDirGeminiFile))} ---`,
    ...
    it('should ignore specified directories during downward scan', async () => {
        ...
        const result = flattenResult(await loadServerHierarchicalMemory(cwd, [], false, new FileDiscoveryService(projectRoot), new SimpleExtensionLoader([]), DEFAULT_FOLDER_TRUST, 'tree', {
...
--- End of Context from: ${normMarker(path.relative(cwd, regularSubDirGeminiFile))} ---`,
    ...
    it('should load extension context file paths', async () => {
        ...
        const result = flattenResult(await loadServerHierarchicalMemory(cwd, [], false, new FileDiscoveryService(projectRoot), new SimpleExtensionLoader([
            {
...
--- End of Context from: ${normMarker(path.relative(cwd, extensionFilePath))} ---`,
    ...
    it('should load memory from included directories', async () => {
        ...
        const result = flattenResult(await loadServerHierarchicalMemory(cwd, [includedDir], false, new FileDiscoveryService(projectRoot), new SimpleExtensionLoader([]), DEFAULT_FOLDER_TRUST));
        expect(result).toEqual({
            memoryContent: `--- Project ---
...
--- End of Context from: ${normMarker(path.relative(cwd, includedFile))} ---`,
    ...
    it('should handle multiple directories and files in parallel correctly', async () => {
        ...
        const result = flattenResult(await loadServerHierarchicalMemory(cwd, createdFiles.map((f) => path.dirname(f)), false, new FileDiscoveryService(projectRoot), new SimpleExtensionLoader([]), DEFAULT_FOLDER_TRUST));
