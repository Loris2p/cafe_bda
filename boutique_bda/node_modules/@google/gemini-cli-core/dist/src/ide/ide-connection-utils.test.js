...
import { getConnectionConfigFromFile, validateWorkspacePath, getIdeServerHost, } from './ide-connection-utils.js';
...
describe('ide-connection-utils', () => {
    ...
    describe('getConnectionConfigFromFile', () => {
        ...
        it('should fall back to a different PID if it matches the current workspace', async () => {
            ...
            vi.mocked(fs.promises.readFile).mockRejectedValueOnce(new Error('not found'));
            vi.mocked(fs.promises.readdir).mockResolvedValue([`gemini-ide-server-${otherPid}-111.json`]);
            ...
            expect(result).toEqual(validConfig);
            expect(fs.promises.readFile).toHaveBeenCalledWith(path.join('/tmp', 'gemini', 'ide', `gemini-ide-server-${otherPid}-111.json`), 'utf8');
        });
        it('should prioritize the target PID over other PIDs', async () => {
            ...
            expect(fs.promises.readFile).toHaveBeenCalledWith(path.join('/tmp', 'gemini', 'ide', `gemini-ide-server-${targetPid}-1.json`), 'utf8');
        });
        it('should prioritize an alive process over a dead one', async () => {
            ...
            expect(fs.promises.readFile).toHaveBeenCalledWith(path.join('/tmp', 'gemini', 'ide', `gemini-ide-server-${alivePid}-1.json`), 'utf8');
        });
        it('should prioritize the largest PID (newest) among alive processes', async () => {
            ...
            expect(fs.promises.readFile).toHaveBeenCalledWith(path.join('/tmp', 'gemini', 'ide', `gemini-ide-server-${newPid}-1.json`), 'utf8');
        });
    ...
    describe('validateWorkspacePath', () => {
        it('should return valid if path is within cwd', () => {
            const result = validateWorkspacePath('/test/workspace', '/test/workspace/sub-dir');
        ...
        it('should return invalid if cwd is not within workspace path', () => {
            const result = validateWorkspacePath('/other/workspace', '/test/workspace/sub-dir');
            expect(result.isValid).toBe(false);
...
//# sourceMappingURL=ide-connection-utils.test.js.map