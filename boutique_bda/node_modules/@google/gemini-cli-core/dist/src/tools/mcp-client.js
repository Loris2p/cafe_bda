    ...
    async refreshResources() {
        ...
        try {
            do {
                ...
                clearTimeout(timeoutId);
                this.cliConfig.emitMcpDiagnostic('info', `Resources updated for server: ${this.serverName}`, undefined, this.serverName);
    ...
    async refreshPrompts() {
        ...
        try {
            do {
                ...
                clearTimeout(timeoutId);
                this.cliConfig.emitMcpDiagnostic('info', `Prompts updated for server: ${this.serverName}`, undefined, this.serverName);
    ...
    async refreshTools() {
        ...
        try {
            do {
                ...
                clearTimeout(timeoutId);
                this.cliConfig.emitMcpDiagnostic('info', `Tools updated for server: ${this.serverName}`, undefined, this.serverName);
...
async function handleAutomaticOAuth(mcpServerName, mcpServerConfig, wwwAuthenticate, cliConfig) {
    try {
        ...
        if (!oauthConfig) {
            cliConfig.emitMcpDiagnostic('error', `Could not configure OAuth for '${mcpServerName}' - please authenticate manually with /mcp auth ${mcpServerName}`, undefined, mcpServerName);
            return false;
        }
        ...
        cliConfig.emitMcpDiagnostic('error', `Failed to handle automatic OAuth for server '${mcpServerName}': ${getErrorMessage(error)}`, error, mcpServerName);
        return false;
    }
}
/**
...
async function createTransportWithOAuth(mcpServerName, mcpServerConfig, accessToken, cliConfig) {
    try {
        ...
        cliConfig.emitMcpDiagnostic('error', `Failed to create OAuth transport for server '${mcpServerName}': ${getErrorMessage(error)}`, error, mcpServerName);
        return null;
    }
}
/**
...
export async function connectAndDiscover(clientVersion, mcpServerName, mcpServerConfig, toolRegistry, promptRegistry, debugMode, workspaceContext, cliConfig) {
    ...
    try {
        ...
        mcpClient.onerror = (error) => {
            cliConfig.emitMcpDiagnostic('error', `MCP ERROR (${mcpServerName}):`, error, mcpServerName);
        ...
        cliConfig.emitMcpDiagnostic('error', `Error connecting to MCP server '${mcpServerName}': ${getErrorMessage(error)}`, error, mcpServerName);
        updateMCPServerStatus(mcpServerName, MCPServerStatus.DISCONNECTED);
    }
}
/**
    ...
    try {
        ...
        for (const toolDef of response.tools) {
            try {
                ...
                cliConfig.emitMcpDiagnostic('error', `Error discovering tool: '${toolDef.name
                // eslint-disable-next-line @typescript-eslint/no-unsafe-type-assertion
                }' from MCP server '${mcpServerName}': ${error.message}`, error, mcpServerName);
            }
        }
        ...
        if (error instanceof Error &&
            ...
            cliConfig.emitMcpDiagnostic('error', `Error discovering tools from ${mcpServerName}: ${getErrorMessage(error)}`, error, mcpServerName);
        }
        return [];
    }
}
 ...
 * @param mcpClient The active MCP client instance.
 */
export async function discoverPrompts(mcpServerName, mcpClient, cliConfig, options) {
    ...
    try {
        ...
        cliConfig.emitMcpDiagnostic('error', `Error discovering prompts from ${mcpServerName}: ${getErrorMessage(error)}`, error, mcpServerName);
        throw error;
    }
}
export async function discoverResources(mcpServerName, mcpClient, cliConfig) {
...
async function listResources(mcpServerName, mcpClient, cliConfig) {
    ...
    try {
        ...
        cliConfig.emitMcpDiagnostic('error', `Error discovering resources from ${mcpServerName}: ${getErrorMessage(error)}`, error, mcpServerName);
        throw error;
    }
    return resources;
}
 ...
 */
export async function invokeMcpPrompt(mcpServerName, mcpClient, promptName, promptParams, cliConfig) {
    ...
    try {
        ...
        if (error instanceof Error &&
            !error.message?.includes('Method not found')) {
            cliConfig.emitMcpDiagnostic('error', `Error invoking prompt '${promptName}' from ${mcpServerName} ${promptParams}: ${getErrorMessage(error)}`, error, mcpServerName);
        }
        throw error;
    }
}
 ...
 * @param cliConfig The CLI configuration providing sanitization and diagnostic reporting
 */
async function retryWithOAuth(client, serverName, config, accessToken, httpReturned404, cliConfig) {
    if (httpReturned404) {
    ...
    const httpTransport = await createTransportWithOAuth(serverName, config, accessToken, cliConfig);
    if (!httpTransport) {
 ...
 */
export async function connectToMcpServer(clientVersion, mcpServerName, mcpServerConfig, debugMode, workspaceContext, cliConfig) {
...
/** Visible for Testing */
export async function createTransport(mcpServerName, mcpServerConfig, debugMode, cliConfig) {
        ...
        if (authProvider === undefined) {
            ...
            if (mcpServerConfig.oauth?.enabled && mcpServerConfig.oauth) {
                ...
                if (!accessToken) {
                    ...
                    cliConfig.emitMcpDiagnostic('info', `MCP server '${mcpServerName}' requires authentication using: /mcp auth ${mcpServerName}`, undefined, mcpServerName);
                }
            }
            else {
