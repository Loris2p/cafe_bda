 ...
 */
export function groupMatchesByFile(allMatches) {
    const groups = {};
 ...
 */
export async function readFileLines(absolutePath) {
    try {
 ...
 */
export async function enrichWithAutoContext(matchesByFile, matchCount, params) {
        ...
        for (const filePath in matchesByFile) {
            ...
            if (fileLines) {
                ...
                for (const match of fileMatches) {
                    ...
                    const endLine = Math.min(fileLines.length, match.lineNumber - 1 + contextLines + 1);
                ...
                matchesByFile[filePath] = newFileMatches.sort((a, b) => a.lineNumber - b.lineNumber);
            }
        }
    }
}
 ...
 */
export async function formatGrepResults(allMatches, params, searchLocationDescription, totalMaxMatches) {
    ...
    if (names_only) {
        ...
        let llmContent = `Found ${filePaths.length} files with matches for pattern "${pattern}" ${searchLocationDescription}${include_pattern ? ` (filter: "${include_pattern}")` : ''}${wasTruncated
...
//# sourceMappingURL=grep-utils.js.map