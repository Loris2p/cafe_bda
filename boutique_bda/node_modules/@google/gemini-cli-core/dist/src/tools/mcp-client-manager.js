        ...
        const currentDiscoveryPromise = new Promise((resolve, reject) => {
            (async () => {
                try {
                    ...
                    const client = new McpClient(name, config, this.toolRegistry, this.cliConfig.getPromptRegistry(), this.cliConfig.getResourceRegistry(), this.cliConfig.getWorkspaceContext(), this.cliConfig, this.cliConfig.getDebugMode(), this.clientVersion, async () => {
                    ...
                    try {
                        ...
                        if (!isAuthenticationError(error)) {
                            ...
                            this.emitDiagnostic('error', `Error during discovery for MCP server '${name}': ${errorMessage}`, error);
                        }
                    }
                }
                catch (error) {
                    const errorMessage = getErrorMessage(error);
                    this.emitDiagnostic('error', `Error initializing MCP server '${name}': ${errorMessage}`, error);
                }
                finally {
                    resolve();
                }
