...
describe('McpClientManager', () => {
    ...
    describe('diagnostic reporting', () => {
        ...
        it('should emit hint instead of full error when user has not interacted with MCP', () => {
            ...
            expect(coreEventsMock.emitFeedback).toHaveBeenCalledWith('info', 'MCP issues detected. Run /mcp list for status.');
            expect(coreEventsMock.emitFeedback).not.toHaveBeenCalledWith('error', 'Something went wrong', expect.anything());
        });
        it('should emit full error when user has interacted with MCP', () => {
            const manager = new McpClientManager('0.0.1', toolRegistry, mockConfig);
            manager.setUserInteractedWithMcp();
            manager.emitDiagnostic('error', 'Something went wrong', new Error('boom'));
            expect(coreEventsMock.emitFeedback).toHaveBeenCalledWith('error', 'Something went wrong', expect.any(Error));
        });
        ...
        it('should capture last error for a server even when silenced', () => {
            const manager = new McpClientManager('0.0.1', toolRegistry, mockConfig);
            manager.emitDiagnostic('error', 'Error in server (test-server)', undefined, 'test-server');
